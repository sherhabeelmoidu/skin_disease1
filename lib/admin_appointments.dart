...
class _AdminAppointmentsState extends State<AdminAppointments> {
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          elevation: 0,
          title: const Text('Appointments Management', style: TextStyle(color: Color(0xFF2C3E50), fontWeight: FontWeight.bold)),
          bottom: const TabBar(
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text('Patient: ${data['userName'] ?? 'Unknown'}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
              _buildStatusLabel(status),
            ],
          ),
          const SizedBox(height: 8),
          Text('Doctor: Dr. ${data['doctorName']}', style: const TextStyle(color: Color(0xFF3B9AE1), fontSize: 14)),
  ...
  Future<void> _updateStatus(String docId, String newStatus) async {
    await _firestore.collection('appointments').doc(docId).update({'status': newStatus});
  }
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            TextField(controller: treatmentController, decoration: const InputDecoration(labelText: 'Treatment Given'), maxLines: 2),
            TextField(controller: tipsController, decoration: const InputDecoration(labelText: 'Doctor Tips'), maxLines: 2),
          ],
        ),
        actions: [
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Patient Progress Updates', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            const SizedBox(height: 20),
            Expanded(
              child: StreamBuilder<QuerySnapshot>(
                stream: _firestore.collection('appointments').doc(appointmentId).collection('progress_updates').orderBy('timestamp', descending: true).snapshots(),
                builder: (context, snapshot) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return _buildProgressUpdateItem(appointmentId, doc.id, update);
                    },
                  );
                },
              ),
  ...
  Widget _buildProgressUpdateItem(String appointmentId, String updateId, Map<String, dynamic> update) {
    final feedbackController = TextEditingController(text: update['doctorFeedback'] ?? '');
          ...
          TextField(
            ...
            decoration: InputDecoration(
              ...
              suffixIcon: IconButton(
                ...
                onPressed: () async {
                  await _firestore.collection('appointments').doc(appointmentId).collection('progress_updates').doc(updateId).update({
                    'doctorFeedback': feedbackController.text,
                  });
